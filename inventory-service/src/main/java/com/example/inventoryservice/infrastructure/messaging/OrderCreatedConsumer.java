 ...
 * Listens to order.created routing key and reserves stock.
...
public class OrderCreatedConsumer {
    ...
    public OrderCreatedConsumer(ReserveStockUseCase reserveStockUseCase, ObjectMapper objectMapper) {
     ...
     */
    @RabbitListener(queues = "${rabbitmq.queue.order-created:inventory.order.created.queue}", ackMode = "MANUAL")
    public void handleOrderCreated(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) {
        ...
        try {
            ...
            try {
                ...
                logger.info("Successfully processed stock reservation for order: {}", event.getPayload().getOrderId());
                ...
                logger.error("Business logic error processing stock reservation for order: {}, eventId: {}, error: {}", 
